...
describe('Refresh token middleware API tests', () => {
  let user;
  ...
  it('should reject when refresh token is expired', async () => {
    const refreshToken = tokenService.createToken(user.toJSON(), 0);
    const { body: responseBody } = await supertest(app)
      .post(`${TOKEN.TOKEN_ROUTER}${TOKEN.REFRESH_TOKEN}`)
  ...
  it('should reject when refresh token is malformed', async () => {
    const refreshToken = tokenService.createToken(user.toJSON(), '1h');
    const { body: responseBody } = await supertest(app)
      .post(`${TOKEN.TOKEN_ROUTER}${TOKEN.REFRESH_TOKEN}`)
      .set('authorization', `Bearer ${refreshToken.repeat(2)}`)
      .expect(UNAUTHORIZED);
  ...
  it('should reject when refresh token is not expired but not present in database', async () => {
    const refreshToken = tokenService.createToken(user.toJSON(), '1h');
    const { body: responseBody } = await supertest(app)
      .post(`${TOKEN.TOKEN_ROUTER}${TOKEN.REFRESH_TOKEN}`)
      .set('authorization', `Bearer ${refreshToken}`)
  ...
  it('should reject when refresh token is not expired and present in database but user with which it associates does not exist in database', async () => {
  ...
  it('should not reject when refresh token is not expired and present in database and user with which is associates also exists', async () => {
    const accessToken = tokenService.createAccessToken(user.toJSON());
    const refreshToken = tokenService.createRefreshToken(user.toJSON());
